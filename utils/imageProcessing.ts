import type { SelectionRect, FootDataSide, FootMetrics, Point, FootSide } from './types.ts';
...
export function calculateRearfootAngle(lines: { calf: [Point, Point], heel: [Point, Point] }, side: FootSide): number {
    ...
    // Ensure points are ordered top-to-bottom for consistent vector direction
    const calfLine = lines.calf[0].y < lines.calf[1].y ? lines.calf : [lines.calf[1], lines.calf[0]];
    const heelLine = lines.heel[0].y < lines.heel[1].y ? lines.heel : [lines.heel[1], lines.heel[0]];

    const angleCalf = getAngle(calfLine[0], calfLine[1]);
    const angleHeel = getAngle(heelLine[0], heelLine[1]);
    ...
    // Clinical convention normalization:
    // Positive (+) is Valgus (eversion, heel deviates laterally/outward)
    // Negative (-) is Varus (inversion, heel deviates medially/inward)
    if (side === 'right') {
        return -angleDiff;
    }
    return angleDiff;
...
export async function calculateAllMetrics(footData: FootDataSide, side: FootSide): Promise<FootMetrics> {
    ...
    const contactArea = getContactArea(footData);
    const weightedPressure = contactArea && contactArea.area > 0 ? metrics.totalLoad / contactArea.area : 0;
