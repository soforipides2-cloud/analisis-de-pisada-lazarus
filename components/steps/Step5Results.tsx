...
import type { PatientData, FootData, FootMetrics, ReportData, FootDataSide, Point } from '../../types.ts';
import { generateExecutiveSummary } from '../../services/geminiService.ts';
import { calculateAllMetrics } from '../../utils/imageProcessing.ts';
...
const Step5Results: React.FC<{ patientData: PatientData; footData: FootData; onBack: () => void; }> = ({ patientData, footData, onBack }) => {
    ...
    const getBalanceColor = (balance: number): 'green' | 'yellow' | 'red' => {
        if (balance >= 90) return 'green';
        if (balance >= 80) return 'yellow';
        return 'red';
    };
    ...
    const SymmetryRow: React.FC<{
        ...
        unit: string;
    }> = ({ label, valueRight, valueLeft, unit }) => {
        const hasValues = valueRight !== null && valueLeft !== null && valueRight > 0 && valueLeft > 0;
        const balance = hasValues ? (Math.min(valueRight!, valueLeft!) / Math.max(valueRight!, valueLeft!)) * 100 : 0;
        ...
        return (
            <tr className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                    {valueRight !== null ? `${valueRight.toFixed(2)}${unit}` : 'N/A'}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                    {valueLeft !== null ? `${valueLeft.toFixed(2)}${unit}` : 'N/A'}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-right">
                    <div className="flex justify-end items-center">
                        <span className="font-semibold">{hasValues ? `${balance.toFixed(0)}%` : 'N/A'}</span>
                        <div className={`ml-3 traffic-light-indicator traffic-light-${color}`}></div>
                    </div>
                </td>
            </tr>
        );
                        ...
                        <div className="overflow-x-auto border rounded-lg">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Parámetro</th>
                                        <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Pie Derecho</th>
                                        <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Pie Izquierdo</th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    <SymmetryRow label="Área de Contacto" valueRight={metrics.right.contactArea?.area} valueLeft={metrics.left.contactArea?.area} unit=" cm²" />
                                    <SymmetryRow label="Carga Total (Presión)" valueRight={metrics.right.totalLoad / 1000} valueLeft={metrics.left.totalLoad / 1000} unit=" k" />
                                    <SymmetryRow label="Presión Ponderada" valueRight={metrics.right.weightedPressure / 1000} valueLeft={metrics.left.weightedPressure / 1000} unit=" k/cm²" />
                                    <SymmetryRow label="Simetría Arco (Ángulo)" valueRight={metrics.right.navicularAngle} valueLeft={metrics.left.navicularAngle} unit="°" />
                                    <SymmetryRow label="Simetría Talón (Ángulo)" valueRight={metrics.right.rearfootAngle} valueLeft={metrics.left.rearfootAngle} unit="°" />
                                    <SymmetryRow label="Simetría Carga Retropié" valueRight={metrics.right.retropie} valueLeft={metrics.left.retropie} unit="%" />
                                    <SymmetryRow label="Simetría Carga Mediopié" valueRight={metrics.right.mediopie} valueLeft={metrics.left.mediopie} unit="%" />
                                    <SymmetryRow label="Simetría Carga Antepié" valueRight={metrics.right.antepie} valueLeft={metrics.left.antepie} unit="%" />
                                </tbody>
                            </table>
                        </div>
                    </section>
